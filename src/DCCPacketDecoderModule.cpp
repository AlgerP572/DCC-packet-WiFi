...
bool INTERRUPT_SAFE capture(unsigned long halfBitLengthTicks)
{
    ...
    intervalCount++;
    dccIntervals[activePacket][intervalCount] = interruptInterval;    
    ...
    if (altbit)
    {
        ...
        if (!preambleFound)
        {
            ...
            { // preambleOneCount >= 10 and bitValue==0
        ...
        else
        {
             ...
             // Preamble previously found, so this is a message bit
            ...
            else
            {
                ...
                if (inputBitCount == 8)
                {
                    // Byte previously completed, so this bit is
                    // the interbyte marker
                    if (bitValue == 0)
                    { // Interbyte marker is zero, so prepare for next
                      // byte of data
                        inputBitCount = 0;
                    }
                    else
                    { 
                        // one-bit found, marks end of packet
                        // End of packet found
                        dccPacket[activePacket][0] = inputByteNumber;  // save number of bytes
                        dccIntervals[activePacket][0] = intervalCount; // save number of intervals
...
void DCCPacketDecoderModule::printPacketBits(Print &output, int index)
{
    output.print(' ');
...
//=======================================================================
// PrintPacketIntervals prints the interval timings for the packet bits. 
void DCCPacketDecoderModule::printPacketIntervals(Print &output, int index)
{  
    output.print(' ');  
    for (byte i = 0; i < dccIntervals[index][0]; i++)
    { 
        output.print(' ');      
        unsigned int interval = dccIntervals[index][i];       
...
void DCCPacketDecoderModule::clearHashList()
{
...
bool DCCPacketDecoderModule::processDCC(Print &output)
{
    ...
    if (pktByteCount > 0)
    {
        ...
        if (checksum)
        {
            ...
            // Result should be zero, if not it's an error!
...
void DCCPacketDecoderModule::DecodePacket(Print &output, int inputPacket, bool isDifferentPacket)
{
    ...
    else if (!bitRead(dccPacket[inputPacket][1], 7))
    {
    ...
    {
        if (bitRead(dccPacket[inputPacket][1], 6))
        {

            // bit7=1 AND bit6=1 -> Loc Decoder Long Address
            decoderAddress = 256 *
                (dccPacket[inputPacket][1] & 0B00111111) +
        ...
        else
        {
            ...
            // bit7=1 AND bit6=0 -> Accessory Decoder
    ...
    if (decoderType == 1)
    {
        ...
        if (showAcc)
        {
            ...
            else
            {
                ...
                // Accessory Extended NMRA spec is not clear about address and
                // instruction format !!!
    ...
    if (outputDecodedData)
    {
        ...
        // StringBuilder sbIntervals(intervalBuffer, sizeof(intervalBuffer));
        // printPacketIntervals(sbIntervals, inputPacket);
        // char* intervals = sbIntervals.getString();

        DCCPackets::SetIntervals(inputPacket, dccIntervals[inputPacket]);        
...
void DCCPacketDecoderModule::GetDCCPacketStats(String& jsonData)
{
    ...
    Stats["refreshTime"] = stats.refreshTime;

    Stats["count"] = stats.count;
    Stats["count0"] = stats.count0;
    Stats["count1"] = stats.count1;
    Stats["glitchCount"] = stats.glitchCount; 

    Stats["packetCount"] = stats.packetCount;
    Stats["outOfSpecRejectionCount"] = stats.outOfSpecRejectionCount;
    Stats["checksumError"] = stats.checksumError;
    Stats["countLostPackets"] = stats.countLostPackets;
    Stats["countLongPackets"] = stats.countLongPackets;

    Stats["max0"] = stats.max0;
    Stats["min0"] = stats.min0;
    Stats["max0BitDelta"] = stats.max0BitDelta;
    Stats["total0"] = stats.total0;

    Stats["max1"] = stats.max1;
    Stats["min1"] = stats.min1;
...
void DCCPacketDecoderModule::GetDCCPacketBytes(String& jsonData)
{
    ...
    Packets["PacketZero"] = DCCPackets::GetPacketSrting(0);
    Packets["PacketOne"] = DCCPackets::GetPacketSrting(1);
    ...
    for (int i = 0; i <= DCCPackets::GetIntervalLength(0); i++)
 	{
        PacketZeroInterval.add(intervals[i]);
    }

    serializeJson(doc, jsonData);
}

unsigned int DCCPacketDecoderModule::GetRefreshDelay()
{
